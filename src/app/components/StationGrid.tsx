...
<p className="text-slate-500 dark:text-slate-400 mt-1">{stations.length>0?`${stations.length} Stations found  worldwide`:''}</p>
</div>
...
<div className="absolute inset-0 bg-primary/20 rounded-2xl flex items-center justify-center opacity-0 group-hover:opacity-100 transition-opacity">
   {!isActive ? <span className="material-icons-round text-4xl text-white"><FaPlayCircle/></span>  : ''}
</div>
    
<div className="absolute top-2 right-2">
<button className="p-2 bg-black/30 backdrop-blur-md rounded-full text-white hover:text-red-400 transition-colors" onClick={(e) => toggleFavoriteHandler(e, station)}>
<span className="material-icons-round text-xl"> {fav ? <FaStar size={16} /> : <FaRegStar size={16} />}</span>
</button>
</div>
</div>
...
<p className="text-xs text-slate-500 dark:text-slate-400 uppercase tracking-widest font-semibold"> {station.language || "India"} • Live</p>
</div>
